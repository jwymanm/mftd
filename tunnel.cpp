...
void* tunnel(void* arg) {
  ...
  if (config.logging && strcmp(config.logging, "None"))
    dolog = atoi(config.logging);

	if (build_server() == 1) { tunnel_cleanup(0,0,1); }

  debug(0, "Tunnel Status: Running", NULL);

	do { if (wait_for_clients() == 0)  handle_client(); }
	while (stayalive);
