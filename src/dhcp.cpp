...
int cleanup(int et) {
  logMesg("DHCP closing network connection", LOG_INFO);
  closeConn();
    ...
    if (select(network.maxFD, &readfds, NULL, NULL, &lb.tv)) {
      ...
      if (net.ready) {
        ...
        if (network.httpConn.ready && FD_ISSET(network.httpConn.sock, &readfds)) {
          ...
          } else logMesg("DHCP memory error", LOG_INFO);
        }
        for (int i = 0; i < MAX_SERVERS && network.dhcpConn[i].ready; i++) {
...
void procHTTP(data19 *req) {
  ...
  if (errno || req->bytes <= 0) {
    ...
    free(req);
    return;
  } else if (config.verbose || config.logging >= 2) {
  ...
  if (cfig.httpClients[0] && !findServer(cfig.httpClients, 8, req->remote.sin_addr.s_addr)) {
    if (config.verbose || config.logging >= 2) {
  ...
  if (fp && !strcasecmp(fp, "/")) sendStatus(req);
    ...
    if (fp && (config.verbose || config.logging >= 2)) {
...
void sendStatus(data19 *req) {
  ...
  fp += sprintf(fp, bodyStart, gb.displayName);
  fp += sprintf(fp, "<table border=\"1\" cellpadding=\"1\" width=\"640\" bgcolor=\"#ccc\">\n");
...
MYDWORD resad(data9 *req) {
  ...
  if (req->dhcpEntry && req->dhcpEntry->fixed) {
    if (req->dhcpEntry->ip) {
      ...
      if (config.verbose || config.logging) {
  ...
  if (config.verbose || config.logging) {
	...
	sprintf(lb.log, "No free leases for DHCPDISCOVER for %s (%s) from RelayAgent %s", req->chaddr, req->hostname, IP2String(lb.tmp, req->dhcpp.header.bp_giaddr));
        ...
        sprintf(lb.log, "No free leases for DHCPDISCOVER for %s (%s) from interface %s", req->chaddr, req->hostname, IP2String(lb.tmp, network.dhcpConn[req->sockInd].server));
    } else {
      if (req->dhcpp.header.bp_giaddr)
	sprintf(lb.log, "No Matching DHCP Range for DHCPDISCOVER for %s (%s) from RelayAgent %s", req->chaddr, req->hostname, IP2String(lb.tmp, req->dhcpp.header.bp_giaddr));
        ...
        sprintf(lb.log, "No Matching DHCP Range for DHCPDISCOVER for %s (%s) from interface %s", req->chaddr, req->hostname, IP2String(lb.tmp, network.dhcpConn[req->sockInd].server));
    }
    logMesg(lb.log, 1);
  }
  return 0;
...
MYDWORD sdmess(data9 *req) {
  ...
  if (req->req_type == DHCP_MESS_DISCOVER && !req->dhcpp.header.bp_giaddr) {
    req->bytes = sendto(network.dhcpConn[req->sockInd].sock, req->raw, packSize, MSG_DONTROUTE, (sockaddr*)&req->remote, sizeof(req->remote));
  } else {
    req->bytes = sendto(network.dhcpConn[req->sockInd].sock, req->raw, packSize, 0, (sockaddr*)&req->remote, sizeof(req->remote));
  }
...
MYWORD gdmess(data9 *req, MYBYTE sockInd) {
  ...
  req->bytes = recvfrom(network.dhcpConn[req->sockInd].sock, req->raw, sizeof(req->raw), 0, (sockaddr*)&req->remote, &req->sockLen);
  ...
  if (!req->subnetIP) {
    ...
    else req->subnetIP = network.dhcpConn[req->sockInd].server;
  }
  ...
  if (config.logging >= 2 || config.verbose) {
    if (req->req_type == DHCP_MESS_NONE) {
      if (req->dhcpp.header.bp_giaddr)
        ...
        sprintf(lb.log, "BOOTPREQUEST for %s (%s) from interface %s received", req->chaddr, req->hostname, IP2String(lb.tmp, network.dhcpConn[req->sockInd].server));
      logMesg(lb.log, 2);
      ...
      if (req->dhcpp.header.bp_giaddr)
        ...
        sprintf(lb.log, "DHCPDISCOVER for %s (%s) from interface %s received", req->chaddr, req->hostname, IP2String(lb.tmp, network.dhcpConn[req->sockInd].server));
      logMesg(lb.log, 2);
    } else if (req->req_type == DHCP_MESS_REQUEST) {
      if (req->dhcpp.header.bp_giaddr)
        ...
        sprintf(lb.log, "DHCPREQUEST for %s (%s) from interface %s received", req->chaddr, req->hostname, IP2String(lb.tmp, network.dhcpConn[req->sockInd].server));
