...
MYDWORD alad(data9 *req) {
  ...
  if (req->dhcpEntry && (req->req_type == DHCP_MESS_NONE || req->resp_type == DHCP_MESS_ACK)) {
    ...
    if (cfig.replication && cfig.dhcpRepl > t) sendRepl(req);
...
void setLeaseExpiry(data7 *dhcpEntry, MYDWORD lease) {
  if (dhcpEntry && dhcpEntry->ip) {
    if (lease > (MYDWORD)(INT_MAX - t)) dhcpEntry->expiry = INT_MAX;
    else dhcpEntry->expiry = t + lease;
...
void __cdecl init(void *lpParam) {
  ...
  fEvent = CreateEvent(
    ...
    FALSE,  // ManualReset
    TRUE,   // Signalled
    TEXT(NAME "FileEvent"));  // object name
  ...
  lEvent = CreateEvent(
    ...
    FALSE,                 // ManualReset
    TRUE,                  // Signalled
    TEXT(NAME "LogEvent"));  // object name
  ...
  sprintf(logBuff, "Binding Interface..");
  logDHCPMess(logBuff, 1);
  ...
  do {
    ...
    if (!network.dhcpConn[0].ready) {
      sprintf(logBuff, "Interface not ready, Waiting...");
      logDHCPMess(logBuff, 1);
      continue;
    }
...
bool detectChange() {
  ...
  if (cfig.failureCount) {
    ...
    Sleep(eventWait);
    sprintf(logBuff, "Retrying failed interface..");
    logDHCPMess(logBuff, 1);
    network.ready = false;
  ...
  sprintf(logBuff, "Network changed, re-detecting interface..");
  logDHCPMess(logBuff, 1);
...
void logDHCPMess(char *logBuff, MYBYTE logLevel) {
  if (verbatim) debug(0, "DHCP: ", (void* ) logBuff);
